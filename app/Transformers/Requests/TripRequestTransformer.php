...
class TripRequestTransformer extends Transformer
{
    ...
    public function transform(RequestModel $request)
    {
        $params =  [
            ...
            'is_later' => intval($request->is_later),
            ...
            'is_driver_started'=>intval($request->is_driver_started),
            'is_driver_arrived'=>intval($request->is_driver_arrived),
            'updated_at'=>$request->converted_updated_at,
            'is_trip_start'=>intval($request->is_trip_start),
            'total_distance'=>doubleval(number_format($request->total_distance,2)),
            'total_time'=>doubleval($request->total_time),
            'is_completed'=>intval($request->is_completed),
            'is_cancelled' =>intval( $request->is_cancelled),
            'cancel_method'=>intval($request->cancel_method),
            'payment_opt' =>intval( $request->payment_opt),
            ...
            'pick_lat'=>doubleval($request->pick_lat),
            'pick_lng'=>doubleval($request->pick_lng),
            'drop_lat'=>doubleval($request->drop_lat),
            'drop_lng'=>doubleval($request->drop_lng),
