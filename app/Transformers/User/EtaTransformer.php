...
class EtaTransformer extends Transformer
{
    ...
    public function transform(ZoneType $zone_type)
    {
        ...
        $response['additional_distance_start'] = $ride->additional_distance_start;
        $response['price_per_additional_distance'] = $ride->price_per_additional_distance;
    ...
    private function calculateRideFares($distance_in_unit, $dropoff_time_in_seconds, $zone_type, $type_prices, $coupon_detail)
    {
        ...
        // Asign additional distance variables
        $additional_distance_start = $type_prices->additional_distance_start;
        $price_per_additional_distance = $type_prices->price_per_additional_distance;
        ...
        if($zone_surge_price) {
            ...
            $surge_additional_cost = ($price_per_additional_distance * ($surge_percent / 100));

            $price_per_distance += $surge_price_additional_cost;
            $price_per_additional_distance += $surge_additional_cost;

        }
        $distance_price = ($calculatable_distance * $price_per_distance);

        // Start calculate additional distance price
        if($additional_distance_start && $calculatable_distance > $additional_distance_start) {
            $additional_distance = $calculatable_distance  - $additional_distance_start;
            $distance_price = ($additional_distance_start * $price_per_distance);
            $distance_price += ($additional_distance * $price_per_additional_distance);
        }
        ...
        return (object)[
                ...
                'price_per_distance' => $type_prices->price_per_distance,
                'additional_distance_start' => $additional_distance_start,
                'price_per_additional_distance' => $price_per_additional_distance,
